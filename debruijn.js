...
var PS = PS || {};
PS.Data_Foreign_DeBruijn = (function () {
    "use strict";
    var Prelude = PS.Prelude;
    function unsafeForeignStatement(stmt) {  var re = new RegExp('\\$\\d+', 'g'), result, max = 0;  while (result = re.exec(stmt))    max = Math.max(Number(result[0].substr(1)) + 1, max);  return Function(wrap(max))();  function wrap(n) {    return !n ? stmt : 'return function ($' + (n-1) + ') { ' + wrap(n-1) + ' };';  }};
    var unsafeForeignExpression = function (expr) {
        return unsafeForeignStatement("return " + expr + ";");
    };
    return {
        unsafeForeignExpression: unsafeForeignExpression, 
        unsafeForeignStatement: unsafeForeignStatement
    };
})();
var PS = PS || {};
PS.Data_Eq = (function () {
